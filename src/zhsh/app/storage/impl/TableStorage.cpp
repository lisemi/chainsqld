...
namespace ripple {
    ...
    {      
		...
		ZHSHChainTx& zhshchainTx = dynamic_cast<ZHSHChainTx&>(transactor);
		...
		for (auto& eachTx : vecTxs)
		{
			...
			auto ret = TableStorageHandlePut(zhshchainTx, uTxDBName, accountID, sTableName, lastLedgerSequence, tx.getTransactionID(), eachTx);
	...
	TER TableStorage::TableStorageHandlePut(ZHSHChainTx& transactor,uint160 uTxDBName, AccountID accountID,std::string sTableName,uint32 lastLedgerSequence,uint256 txhash, STTx const & tx)
    {
        std::lock_guard<std::mutex> lock(mutexMap_);
