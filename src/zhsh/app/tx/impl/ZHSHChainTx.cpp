...
#include <zhsh/protocol/TableDefines.h>
...
namespace ripple {
	...
	ZHSHChainTx::ZHSHChainTx(ApplyContext& ctx)
		: Transactor(ctx)
	{

	}

	TER ZHSHChainTx::preflight(PreflightContext const& ctx)
	{
		const STTx & tx = ctx.tx;
		Application& app = ctx.app;
		auto j = app.journal("preflightZHSHChain");
	...
	TER ZHSHChainTx::preclaim(PreclaimContext const& ctx)
	{
		const STTx & tx = ctx.tx;
		Application& app = ctx.app;
		auto j = app.journal("preflightZHSHChain");
		...
		// Check reserve and funds availability
		{
			...
			STAmount priorBalance = STAmount((*sleAccount)[sfBalance]).zhg();
	...
	std::pair<TER, std::string> ZHSHChainTx::dispose(TxStore& txStore, const STTx& tx)
	{
	...
	bool ZHSHChainTx::canDispose(ApplyContext& ctx)
	{
	...
	std::pair<TxStoreDBConn*, TxStore*> ZHSHChainTx::getTransactionDBEnv(ApplyContext& ctx)
	{
