#include <zhsh/app/misc/SleOps.h>
...
#include <zhsh/app/tx/DirectApply.h>
#include <zhsh/app/misc/ContractHelper.h> 
...
#include <zhsh/vm/VMFace.h>

namespace ripple {
    //just raw function for zhg, all paras should be tranformed in extvmFace modules.
	...
	TER SleOps::doPayment(AccountID const& _from, AccountID const& _to, uint256 const& _value)
	{
			...
			[&_from,&_to,&value](auto& obj)
		{
			...
			obj.setFieldAmount(sfAmount, ZHGAmount(value));
		});
	...
	void SleOps::addBalance(AccountID const& addr, int64_t const& amount)
	{
		...
		if (pSle)
		{
			auto balance = pSle->getFieldAmount(sfBalance).zhg().drops();
			int64_t finalBanance = balance + amount;
			if (amount > 0)
			{
				pSle->setFieldAmount(sfBalance, ZHGAmount(finalBanance));
				ctx_.view().update(pSle);
			}			
		}
	}
	...
	int64_t SleOps::balance(AccountID const& address)
	{
		...
		if (pSle)
			return pSle->getFieldAmount(sfBalance).zhg().drops();
		else
			return 0;
	}
